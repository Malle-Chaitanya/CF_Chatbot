...
| **Auth** | Session-based (MongoDB session store) + Microsoft OAuth (login only); Bearer token fallback; admin allowlist (`ADMIN_EMAILS`) |
...
- **require_admin** / **require_restricted_admin** — Admin-only routes; allowlist from `ADMIN_EMAILS` (env).
...
- **Auth**: `MICROSOFT_CLIENT_ID`, `MICROSOFT_CLIENT_SECRET`, `MICROSOFT_TENANT`; `ADMIN_EMAILS`, `EXCLUDED_DEVELOPER_EMAILS`.
